...
import { ShieldAlert, FileSpreadsheet, Sparkles } from 'lucide-react';
...
const AppContent: React.FC = () => {
  const { isAuthenticated, currentPermissions, isGoogleSignedIn, handleGoogleSignIn } = useFinance();
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto flex flex-col">
            <main className="flex-1 p-4 sm:p-6 md:p-8 max-w-7xl w-full mx-auto">
              {!isGoogleSignedIn && (
                <div className="mb-5 p-3.5 sm:p-4 bg-gradient-to-r from-teal-500/10 via-emerald-500/10 to-teal-500/10 dark:from-teal-950/40 dark:via-emerald-950/40 dark:to-teal-950/40 border border-teal-200 dark:border-teal-800/80 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 bg-teal-600 text-white rounded-xl shrink-0 shadow-xs">
                      <FileSpreadsheet className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white text-xs sm:text-sm flex items-center gap-1.5">
                        <span>Connect Google Account for Live Spreadsheet Database Sync</span>
                      </h4>
                      <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5">
                        Sign in to automatically store and update all ledger transactions, budgets, and accounts live in Google Sheet <code className="bg-white dark:bg-slate-900 px-1.5 py-0.5 rounded text-[11px] font-mono border border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-300">13Ceb4ut03DWZ3GUJmMh2uduklLCc1qnn7faRjXV2pac</code>.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={handleGoogleSignIn}
                    className="w-full sm:w-auto px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl text-xs shrink-0 transition-colors shadow-xs flex items-center justify-center gap-1.5 cursor-pointer"
                  >
